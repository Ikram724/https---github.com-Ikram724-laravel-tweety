<div class="flex p-4 border-b border-b-gray-400 border-radius-20">
    <div class="mr-2 flex-shrink-0">
        <a href="{{route('profile',$tweet->user)}}"><img src="{{$tweet->user->avatar}}
    ...
    <div>
        ...
        <div class="mt-2">
            <form method="post" action="/tweet/delete/{{$tweet->id}}">
                @csrf
                @method('DELETE')
                <button class="rounded bg-red-400 text-white hover:bg-red-800 " type="submit">Delete</button>
        ...
        <div class="flex">
            <form method="post" action="/tweets/{{$tweet->id}}/like">
                @csrf
                <div class="flex items-center mt-4 {{$tweet->isLikedBy(auth()->user())? 'text-blue-500':'text-gray-500'}}">
            ...
            <form method="post" action="/tweets/{{$tweet->id}}/dislike">
                @csrf
                @method('DELETE')
                <div class="flex items-center mt-5 ml-4 {{$tweet->notLikedBy(auth()->user())? 'text-blue-500':'text-gray-500'}}">
                    <svg viewBox="0 0 20 20" class="mr-1 w-5">
                        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                            <g class="fill-current">
